...
@rota.get("/contas", response_model=List[ContasResponse], status_code=status.HTTP_200_OK)
def listando_contas_a_pagar_e_receber(db: Session = Depends(get_db)):
    return capturar_contas_a_pagar_e_receber(db)


@rota.get("/contas/{identificador_conta}", response_model=ContasResponse, status_code=status.HTTP_200_OK)
...
@rota.put("/atualizar_contas/{identificador_conta}", response_model=ContasResponse, status_code=status.HTTP_200_OK)
